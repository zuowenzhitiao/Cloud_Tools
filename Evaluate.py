...
def evaluate_metrics(pred_pcl_dir, gt_pcl_dir, gt_mesh_dir, device='cuda'):
    # Load data
    pcls_pred = load_xyz(pred_pcl_dir)
    pcls_gt = load_xyz(gt_pcl_dir)
    meshes = load_off(gt_mesh_dir)

    pcls_pred_name = list(pcls_pred.keys())
    pcls_gt_name = list(pcls_gt.keys())

    results = {}

    for pred_name, gt_name in tqdm(zip(pcls_pred_name, pcls_gt_name), desc='Evaluate'):
        pcl_pred = pcls_pred[pred_name][:, :3].unsqueeze(0).to(device)
        if gt_name not in pcls_gt:
            print(f'Shape `{gt_name}` not found, ignored.')
            continue
        pcl_gt = pcls_gt[gt_name].unsqueeze(0).to(device)
        verts = meshes[gt_name]['verts'].to(device)
        faces = meshes[gt_name]['faces'].to(device)

        cd = pytorch3d.loss.chamfer_distance(pcl_pred, pcl_gt)[0].item()
        cd_sph = chamfer_distance_unit_sphere(pcl_pred, pcl_gt)[0].item()
        hd_sph = hausdorff_distance_unit_sphere(pcl_pred, pcl_gt)[0].item()

        if 'blensor' in pred_name:
    ...
    results_mean = results_df.mean(axis=0)
    results_df.loc['mean'] = results_mean

    results_df.to_csv('evaluation_results.csv', float_format='%.12f')
...
if __name__ == '__main__':
    ...
    results = evaluate_metrics(
        pred_pcl_dir='/home/ubuntu/usrs/JK/Pointfilter-master/Pointfilter-master/Dataset/Result/PCNpulse0.02/xyz',
        gt_pcl_dir='/home/ubuntu/usrs/JK/Pointfilter-master/Pointfilter-master/Dataset/GT/PUNet/pointclouds/test/50000_poisson/xyz',
        gt_mesh_dir='/home/ubuntu/usrs/JK/Pointfilter-master/Pointfilter-master/Dataset/GT/PUNet/meshes/test',
        device='cuda'
    )
    print(results)